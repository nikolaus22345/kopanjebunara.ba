...
**58 MB of source became 7.3 MB of web media.** Photos are JPEG at two widths with correct `width`/`height` so nothing shifts on load. Videos are trimmed to their strongest 13–14 seconds, re-encoded at CRF 31, and **stripped of audio**. Nothing video-related downloads until a visitor clicks a poster.
...
Generated by `node media.mjs`. The heroes and the logo live in the **Downloads root**, not the photo subfolder — the script takes that as a second argument (`node media.mjs "<photos>" "<brand>"`).

| Asset | Source | Note |
|---|---|---|
| `assets/hero/hero-desktop-*.jpg` | `hero desktop.png` 2688×1520 | 1280 / 1920 / 2560w |
| `assets/hero/hero-mobile-*.jpg` | `hero mobile.png` 1520×2688 | 640 / 828 / 1170w |
| `assets/img/logo-96.png`, `logo-192.png` | the square logo PNG | white keyed out to alpha |
| `assets/img/favicon-32/180/512.png` + `favicon.svg` | drawn, not derived | see below |
| `assets/img/og.jpg` | desktop hero + scrim + text | 1200×630 |

**Provenance:** these three came from the client, unlike the scraped material documented above. Treat them as cleared.

### Hero is art-directed, not cropped

Two different photographs. The landscape frame carries the valley and the sky; squeezed into a phone viewport it loses both and becomes a picture of a truck. The portrait frame is composed for that shape and is served below 700px via `<source media>`, so the browser downloads exactly one of them.

Verified: at 375px it fetches `hero-mobile-828.jpg`; on desktop it fetches the landscape file.

### The logo had no alpha

The supplied PNG is **colour-type 2 — RGB, no alpha channel**. The transparent checkerboard you see in an image viewer was flattened onto near-white `#faf7fa`, which on the dark header would have rendered as a white block.

`colorkey` removes the white while keeping the original teal, rather than re-tinting it to a guess. `.brand-mark` was also sized `2rem` square for the old inline SVG and would have squashed a 96×105 image, so it is now height-driven with automatic width.

### The favicon is a redraw, not a downscale

The logo is fine line art. At 16px it averages into a pale smudge — the strata bars and the drop outline stop being distinguishable from the background. Rendering it and inspecting at 16/32/48px confirmed this before anything was changed.

[`src/favicon.mjs`](src/favicon.mjs) keeps the identity — stem, strata bars, water drop — but drops the third bar, thickens every stroke, fills the drop instead of outlining it, and sits it on a dark rounded tile so the mark has presence in a browser tab. Legible at all three sizes.

To adjust it, edit the unit-square constants at the top of that file (`STEM_W`, `BAR_H`, `BARS`, `DROP_CY`, `DROP_R`) and re-run `node media.mjs`.

### The OG card is the real photograph

The earlier card was hand-drawn geometry because nothing here could render type into a raster. That turned out to be unnecessary: ffmpeg has `drawtext`, and Bahnschrift ships with Windows and is the closest system face to the site's Barlow Condensed.

So the card is now the desktop hero with a left-weighted scrim and real text — headline, tagline, phone number, domain. Bosnian diacritics render correctly. A photo of an actual rig at dawn earns a click that an abstract mark does not, and WhatsApp previews are how links get shared in this market.

`src/og-card.html` (the manual-screenshot fallback) is no longer needed but is kept in case you want a different layout.

> **ffmpeg gotcha, if you ever edit this:** an absolute Windows font path cannot go inside a filtergraph. The drive-letter colon is also the filter option separator, and escaping it does not survive ffmpeg's two-stage unescaping. The pipeline copies the font next to the output and runs ffmpeg from that directory, referencing it as a bare filename.

### Which script owns what

`build.mjs` no longer writes `favicon.svg` or `og.png`. Those need ffmpeg and the source art, so they belong to `media.mjs` — and while both wrote them, every site build silently clobbered the good versions with placeholders. If a brand asset ever reverts to something plain, that is the class of bug to look for.
